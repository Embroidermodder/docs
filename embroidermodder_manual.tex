...
\label{the-embroidermodder-team}
...
\include{tables/table_of_format_code_support.tex}
...
\lstinputlisting[language=sh, basicstyle=\ttfamily\small]{scripts/windows_build.bat}
\end{enumerate}
...
\item Get flake8, pylint and tests to pass.
\item Sphinx documentation from docstrings or similar.
\end{enumerate}
...
\input{tables/table_cad_commands.tex}
...
For a full list of members please see Section~\ref{the-embroidermodder-team}
...
\lstinputlisting[language=sh, basicstyle=\ttfamily\small, frame=single]{scripts/libembroidery_build.sh}
...
\lstinputlisting[language=sh, basicstyle=\ttfamily\small, frame=single]{scripts/libembroidery_debugger.sh}

 Note: this is maintained outside of the repository because it gives us a oneliner for new systems with a stable URL:

\begin{verbatim}
curl https://embroidermodder.org/scripts/libembroidery_debugger.sh | sh
\end{verbatim}
...
\begin{enumerate}
\item Currently experimenting with Fritzing$^{8}$, upload netlists to embroiderbot
   when they can run simulations using the asm in `libembroidery`.
\item Create a common assembly for data that is the same across chipsets
   `libembrodiery\_data\_internal.s`.
\item Make the defines part of `embroidery.h` all systems and the function list
...
\begin{enumerate}
\item[0.] \textbf{None} Either the format produces no output, reporting an error. Or it produces a Tajima dst file as an alternative.
\item[1.] \textbf{Poor} A file somewhat similar to our examples is produced. We don't know how well it runs on machines in practice as we don't have any user reports or personal tests.
\item[2.] \textbf{Basic} Simple files in this format run well on machines that use this format.
...
\input{tables/table_of_format_doc_support.tex}
...
\lstinputlisting[basicstyle=\ttfamily\small, frame=single]{samples/example.col}
...
\subsection 1.7.25. Husqvarna Viking Embroidery Format (.hus)

Stitch Only Format.

\subsection 1.7.26. Inbro Embroidery Format (.inb)

Stitch Only Format.

\subsection 1.7.27. Embroidery Color Format (.inf)

Stitch Only Format.

\subsection 1.7.28. Janome Embroidery Format (.jef)

Stitch Only Format.

\subsection 1.7.29. Pfaff professional Design format (.ksm)
...
\subsection{Pfaff PCD File Format}
...
\subsection 1.8.2. Arduino Considerations
...
\subsection{\texttt{convert}}


\chapter{Mobile Support: MobileViewer and iMobileViewer}

\section{Embroidermodder \emversion{} User Manual}
\label{embroidermodder-user-manual}
...
\label{move-a-single-stitch-in-an-existing-pattern}
